...
if ($_GPC["mov"] == "detail") {
    ...
    if (TIMESTAMP > $member["end_time"] && $member["is_pay"] == 1) {
        pdo_update("yingshi_byy_vip_video_member", array(
            "end_time" => null,
            "is_pay" => 0
        ) , array(
            "openid" => $member["openid"]
    ...
    $ad = pdo_fetch("SELECT * FROM " . tablename("yingshi_byy_vip_video_ad") . " WHERE uniacid = :uniacid AND type = 'dumiao'  AND status = 0 ORDER BY rand() DESC LIMIT 1", array(
        ":uniacid" => $_W["uniacid"]
    ) , "id");
    if (TIMESTAMP > $ad["end_time"]) {
        pdo_update("yingshi_byy_vip_video_ad", array(
            "status" => 1
        ) , array(
            "id" => $ad["id"]
        ));
    ...
    $category = pdo_fetchall("SELECT * FROM " . tablename("yingshi_byy_vip_video_category") . " WHERE uniacid = :uniacid AND parentid = :parentid AND status = :status ORDER BY parentid ASC, displayorder ASC, id ASC ", array(
        ":uniacid" => $_W["uniacid"],
        ":parentid" => 0,
        ":status" => 0
    ) , "id");
    ...
    if ($settings["everyday_free_num"] == 1) {
        $num = pdo_fetchcolumn("SELECT COUNT(*),time FROM " . tablename("yingshi_byy_vip_video") . " WHERE uniacid = :uniacid AND openid = :openid AND time >= :firsttime AND time <= :lasttime ", array(
            ":uniacid" => $_W["uniacid"],
            ":openid" => $openid,
    ...
    if ($num >= $settings["free_num"] && $member["is_pay"] == 0) {
        message($settings["warn_font"] ? $settings["warn_font"] : "您的免费观看次数已用完，请点击确定开通会员，无限制观看", link_url("member", array(
            "op" => "open"
        )) , "error");
    }
    $jilu = pdo_fetchall("SELECT openid , yvideo_url FROM " . tablename("yingshi_byy_vip_video") . " WHERE uniacid = :uniacid AND openid = :openid ORDER BY id DESC LIMIT 10", array(
        ":uniacid" => $_W["uniacid"],
        ":openid" => $openid
    ));
    if ($_GPC["d_id"]) {
    ...
    $comment = pdo_fetchall("SELECT * FROM " . tablename("yingshi_byy_vip_video_message") . " WHERE uniacid = :uniacid AND video_id = :video_id AND status = 1 ORDER BY id DESC LIMIT 10", array(
        ":uniacid" => $_W["uniacid"],
        ":video_id" => $video_id
    ));
    $shoucang = pdo_get("yingshi_byy_vip_video", array(
        "uniacid" => $_W["uniacid"],
        "yvideo_url" => $_W["siteurl"],
        "openid" => $openid,
        "type" => "shoucang"
    ));
    if ($id) {
        $content = pdo_fetch("SELECT * FROM " . tablename("yingshi_byy_vip_video_manage") . " WHERE id=:id", array(
            ":id" => $id
        ));
        if (checksubmit("submit")) {
            if ($_GPC["password"] == $content["password"]) {
                setcookie("password", $_GPC["password"], time() + 2 * 7 * 24 * 3600);
                $url = link_url("index", array(
                    "mov" => "detail",
                    "id" => $id
                ));
                Header("Location: " . $url);
        ...
        $is_charge = pdo_get("yingshi_byy_vip_video_order", array(
            "uniacid" => $_W["uniacid"],
            "video_id" => $id
        ));
        $is_vip = is_vip($id, "id");
        pdo_update("yingshi_byy_vip_video_manage", array(
            "click +=" => 1
        ) , array(
            "id" => $id
        ));
        ...
        if ($op == "yule" || $op == "gaoxiao") {
            ...
            if ($settings["ziyuan"] == 1) {
                ...
                $click = pdo_fetchcolumn("SELECT * FROM " . tablename("yingshi_byy_vip_video") . " WHERE uniacid = :uniacid AND video_id = :video_id ", array(
                    ":uniacid" => $_W["uniacid"],
                    ":video_id" => $video_id
                ));
            } else {
                if ($settings["ziyuan"] == 2) {
                        ...
                        if ($_GPC["site"]) {
                            ...
                            if (count($link) > 1) {
                                if (strexists($link["0"], "腾讯") || strexists($link["0"], "华数TV") && !$settings["tengxun"]) {
                                    ...
                                    if (strexists($site_title, "腾讯") || strexists($site_title, "华数TV")) {
                                        $site_title = $link["2"];
                                    } else {
                                        $site_title = $link["0"];
                                    }
                    ...
                    $click = pdo_fetchcolumn("SELECT * FROM " . tablename("yingshi_byy_vip_video") . " WHERE uniacid = :uniacid AND yvideo_url = :yvideo_url ", array(
                        ":uniacid" => $_W["uniacid"],
                        ":yvideo_url" => $yurl
                    ));
                }
    ...
    if (!pdo_get("yingshi_byy_vip_video", array(
        "uniacid" => $_W["uniacid"],
        "openid" => $openid,
        "video_url" => $url
    ))) {
        ...
        if ($id) {
            pdo_insert("yingshi_byy_vip_video", array(
                "uniacid" => $_W["uniacid"],
                "openid" => $openid,
                "title" => $content["title"],
                "video_url" => $url,
    ...
    if ($settings["api"]) {
        ...
        if ($id) {
            if (strexists($url, "zhilian")) {
                ...
                if ($settings["baidu_api"] && strexists($url, "baidu")) {
                    $api = $settings["baidu_api"] . $url;
                } else {
                    if ($Hurl == "https") {
                        $api = "https://jiexi.thecook.com.cn/vip/api.php?url=" . $url . "&link=" . $_GPC["link"];
                    } else {
                        $api = "http://jiexi.thecook.com.cn/vip/api.php?url=" . $url . "&link=" . $_GPC["link"];
                    }
                }
            }
        } else {
            if ($Hurl == "https") {
                $api = "https://jiexi.thecook.com.cn/vip/vip.php?url=" . $url . "&link=" . $_GPC["link"];
            } else {
                $api = "http://jiexi.thecook.com.cn/vip/vip.php?url=" . $url . "&link=" . $_GPC["link"];
            }
        }
    }
    if ($settings["duo_api"]) {
    ...
    if ($type == "shoucang") {
        ...
        $ret = pdo_get("yingshi_byy_vip_video", $data);
        if (empty($ret)) {
            pdo_insert("yingshi_byy_vip_video", $data);
            echo "收藏成功";
            exit;
        } else {
            pdo_delete("yingshi_byy_vip_video", array(
                "id" => $ret["id"]
            ));
            echo "取消收藏成功";
            exit;
    ...
    if ($type == "comment") {
        ...
        if ($data["openid"]) {
            $ret = pdo_insert("yingshi_byy_vip_video_message", $data);
        }
        if (!empty($ret)) {
    ...
    $jilu = pdo_fetchall("SELECT title , yvideo_url FROM " . tablename("yingshi_byy_vip_video") . " WHERE uniacid = :uniacid AND openid = :openid ORDER BY id DESC LIMIT 10", array(
        ":uniacid" => $_W["uniacid"],
        ":openid" => $_W["openid"]
    ));
    if ($site_name) {
    ...
    foreach ($ad as $key => $value) {
        if ($key == "dianying" || $key == "dianshi" || $key == "zongyi" || $key == "dongman") {
            $gg[$key] = pdo_fetch("SELECT * FROM " . tablename("yingshi_byy_vip_video_ad") . " WHERE uniacid = :uniacid AND type = :type  AND status = 0 ORDER BY rand() DESC LIMIT 1", array(
                ":uniacid" => $_W["uniacid"],
                ":type" => $key
            ) , "id");
        } else {
            if ($key != "dumiao") {
                $list_gg[$key] = pdo_fetchall("SELECT * FROM " . tablename("yingshi_byy_vip_video_ad") . " WHERE uniacid = :uniacid AND type = :type  AND status = 0 ORDER BY id DESC ", array(
                    ":uniacid" => $_W["uniacid"],
                    ":type" => $key
                ) , "insert");
            }
    ...
    if ($op == "today") {
        $type = mac_category();
        $today = mac_list(array(
            "op" => "today",
            "pageno" => 100
        ));
    }
    if ($op == "index" && !$type_id_1) {
        $data = pdo_getall("yingshi_byy_vip_video_manage", array(
            "uniacid" => $_W["uniacid"],
            "display !=" => 1
        ) , array() , '', array(
            "sort DESC",
        ...
        if ($settings["ziyuan"] == 2) {
            ...
            $time = cache_load("yingshi_byy_vip_video:time" . $_W["uniacid"]);
            if ($settings["ziyuan"] == 1) {
                $url = m3u8();
                $ip = $_SERVER["SERVER_ADDR"] ? $_SERVER["SERVER_ADDR"] : $_SERVER["REMOTE_ADDR"];
                $url = $url . "?ip=" . $ip . "&op=today&pageno=20";
                $response = ihttp_get($url);
            ...
            if (TIMESTAMP - $time > 3600) {
                ...
                cache_write("yingshi_byy_vip_video:time" . $_W["uniacid"], TIMESTAMP);
                cache_write("yingshi_byy_vip_video:dianying" . $_W["uniacid"], $dianying);
                cache_write("yingshi_byy_vip_video:dianshi" . $_W["uniacid"], $dianshi);
                cache_write("yingshi_byy_vip_video:zongyi" . $_W["uniacid"], $zongyi);
                cache_write("yingshi_byy_vip_video:dongman" . $_W["uniacid"], $dongman);
            } else {
                $dianying = cache_load("yingshi_byy_vip_video:dianying" . $_W["uniacid"]);
                $dianshi = cache_load("yingshi_byy_vip_video:dianshi" . $_W["uniacid"]);
                $zongyi = cache_load("yingshi_byy_vip_video:zongyi" . $_W["uniacid"]);
                $dongman = cache_load("yingshi_byy_vip_video:dongman" . $_W["uniacid"]);
            }
        }
        ...
        foreach ($list_gg["dianying_list"] as $k => $p) {
            array_splice($dianying, $k - 1, 0, array(
                $p
            ));
        }
        ...
        foreach ($list_gg["dianshi_list"] as $k => $p) {
            array_splice($dianshi, $k - 1, 0, array(
                $p
            ));
        }
        ...
        foreach ($list_gg["zongyi_list"] as $k => $p) {
            array_splice($zongyi, $k - 1, 0, array(
                $p
            ));
        }
        ...
        foreach ($list_gg["dongman_list"] as $k => $p) {
            array_splice($dongman, $k - 1, 0, array(
                $p
            ));
        }
        if ($settings["ziyuan"] == 2) {
        ...
        if ($op > 0) {
            ...
            $cat = pdo_getall("yingshi_byy_vip_video_category", array(
                "uniacid" => $_W["uniacid"],
                "parentid" => $op
            ) , array() , '', "id DESC");
        } else {
                ...
                if ($settings["ziyuan"] == 2) {
                    ...
                    if (empty($data) || TIMESTAMP - $discover_time > 3600) {
                        if ($settings["ziyuan"] == 1) {
                            ...
                            $url = m3u8();
                            $ip = $_SERVER["SERVER_ADDR"] ? $_SERVER["SERVER_ADDR"] : $_SERVER["REMOTE_ADDR"];
                            $url = $url . "?ip=" . $ip . "&op=" . $op . "&rank=" . $rank . "&year=" . $year . "&area=" . $area . "&act=" . $act . "&cat=" . $cat . "&pageno=" . $num;
                            $response = ihttp_get($url);
        ...
        if ($_GPC["type"] == "json") {
            if ($op > 0) {
                ...
                $data = pdo_fetchall("SELECT * FROM " . tablename("yingshi_byy_vip_video_manage") . " WHERE uniacid = " . $_W["uniacid"] . " ORDER BY id DESC LIMIT " . ($num - 1) * $pageindex . "," . $pageindex);
                if (!$data) {
                    exit;
                }
            } else {
